...
| 지표 | 측정값 |
|---|---|
| 평균 응답 시간 | 0.7–0.8s (5080 서버) / 1.8s (Jetson 단독) / 0.8s (Jetson→서버) |
| QA 정확도 | 82% (서버) / 67% (Jetson 단독) |
| 메모리 사용량 | 4.2GB (서버) / 1.1GB (Jetson llama.cpp) |
...
**측정 환경 구분** — 아래 세 하드웨어의 수치를 섞어 쓰지 말 것
...
| | 실행 위치 | 추론 엔진 | 평균 응답 | 메모리 | 오프라인 | 개인정보 |
|---|---|---|---|---|---|---|
| Case 1 | RTX 5080 서버 | GPU FastAPI | 0.7–0.8s | 4.2GB | 불가 | 서버 전송 |
| Case 2 | RTX 5080 서버 | llama.cpp | 1.1s | 1.5GB | 불가 | 서버 전송 |
| Case 3 | Jetson → 5080 | 원격 호출 | 0.8s | 4.2GB(서버) | 불가 | 서버 전송 |
| Case 4 | Jetson 단독 | llama.cpp | 1.8s | **1.1GB** | **가능** | **로컬 처리** |
...
| 질문 | 답변 방향 |
|---|---|
| "0.19s와 이력서의 1.4s가 다른데?" | **측정 환경이 다름.** 0.19s는 개인 RTX 3070 웹 데모(2026 재측정, 재현 가능), 1.4s는 회사 Jetson 온디바이스(2025 인턴). 하드웨어 격차가 그대로 반영된 수치. |
| "Jetson 수치는 어떻게 측정했나?" | `test_performance_agx.py`로 종단 시간 측정, `performance_monitor.py`로 CPU/GPU 사용량 모니터링. **당시 측정했으나 원본 로그는 보존하지 못함** — 회사 장비(RTX 5080·Jetson AGX Orin)라 퇴사 후 재현 불가. |
| "5080 실험은 왜 재현이 안 되나?" | 인턴 수행 시 **회사 장비**로 진행. 현재 개인 환경은 RTX 3070이라 동일 조건 재현 불가. 대신 **같은 아키텍처를 3070에서 재구성해 재측정**했고 그 수치는 언제든 재현 가능. |
